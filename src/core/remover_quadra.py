...
def processar_resultados() -> None:
    ...
    with open(RESULTS_FILE, "r", encoding="utf-8") as f:
        ...
        for linha in tqdm(linhas, desc="Processando resultados"):
            ...
            ids_para_deletar = set()  # usar set para evitar duplicatas dentro do concurso
            ...
            for i in tqdm(range(0, len(ids_list), BATCH_SIZE), desc="Deletando jogos", leave=False):
                batch = ids_list[i:i + BATCH_SIZE]
                cursor.executemany("DELETE FROM jogos WHERE id = ?", [(x,) for x in batch])
                conn.commit()
